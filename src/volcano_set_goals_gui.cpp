...
int main(int argc, char **argv) {
    ...
    ros::Subscriber sub_gui_info;
    sub_gui_info = n->subscribe("/gui_info",1,goalCallback);
    sub_qt_info = n->subscribe("/VolcanoGuiInfo",1,qtCallback);
...
void qtCallback(const volcano_robotsim::VolcanoGuiInfo::ConstPtr &value){
    int seat = value->seat_number_on;
    geometry_msgs::PoseStamped target_pose;
    target_pose.header.seq = 1;
    target_pose.header.frame_id = "map";
    switch(seat){
